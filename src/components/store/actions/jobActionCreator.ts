...
export const fetchJobs = ({page = 0}={}) =>(dispatch: Dispatch<any>) => {
...
export const fetchSimilarJobs = ({id = 1}) => (dispatch: Dispatch<any>) => {
    fetch(`https://api.hh.ru/vacancies/${id}/similar_vacancies`).then((res) => {
        return res.json();
    }).then(data => {
